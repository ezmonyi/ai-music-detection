...
def evaluate_locked_sources_generic(
    name: str,
    matrix: np.ndarray,
    rows: list[dict[str, object]],
    seed: int,
) -> list[dict[str, object]]:
    """Evaluate every AI source present instead of assuming Suno source names."""
    ...
    locked_scores = artifactlib.fit_predict(
        matrix[development], labels[development], matrix[locked]
    )
    locked_labels = labels[locked]
    locked_sources = sources[locked]
    ai_sources = sorted(set(sources[labels == 1]))
    output = []
    base_seed = seed + sum((index + 1) * ord(char) for index, char in enumerate(name))
    for source_number, source in enumerate(ai_sources):
        keep = (locked_labels == 0) | ((locked_labels == 1) & (locked_sources == source))
        subset_labels = locked_labels[keep]
        subset_scores = locked_scores[keep]
        if not np.any(subset_labels == 0) or not np.any(subset_labels == 1):
            continue
        balanced_accuracy, roc_auc = artifactlib.metrics(subset_labels, subset_scores)
        ba_low, ba_high, auc_low, auc_high = artifactlib.bootstrap_intervals(
            subset_labels,
            subset_scores,
            base_seed + 10_000 + source_number,
        )
        output.append(
            {
                "representation": name,
...
def main() -> None:
    ...
    for name, matrix in matrices.items():
        ...
        source_evaluations += evaluate_locked_sources_generic(name, matrix, rows, args.seed)
    ...
    for name, matrix in matrices.items():
        ...
        source_evaluations += evaluate_locked_sources_generic(
            sensitivity_name, matrix[activity_mask], active_rows, args.seed + 1_000
        )
