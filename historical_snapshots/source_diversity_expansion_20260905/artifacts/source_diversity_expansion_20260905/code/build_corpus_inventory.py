...
def main():
    ...
    payload = {
        ...
        "global_retained_native_hashes": len({r["raw_sha256"] for r in short if r["raw_sha256"]}),
        "missing_native_hash_records": sum(not r["raw_sha256"] for r in short),
        "development_global_groups": len({r["group_id"] for r in short if r["role"] == "development"}),
    ...
    lines = ["# Frozen corpus inventory", "", f"Snapshot: {payload['generated_utc']}", "",
             ...
             f"Distinct retained-native byte hashes: {payload['global_retained_native_hashes']:,}; "
             f"missing native hashes: {payload['missing_native_hash_records']:,}. "
             f"Global split groups: {payload['global_group_count']:,}; "
             f"development groups: {payload['development_global_groups']:,}.", "",
    ...
    for row in sources:
        ...
        display_name = SOURCE_NAMES.get(row["source"], row["source"])
        lines.append(f"| {display_name} | {row['claimed_label']} | {row['selected_recordings']} | "
