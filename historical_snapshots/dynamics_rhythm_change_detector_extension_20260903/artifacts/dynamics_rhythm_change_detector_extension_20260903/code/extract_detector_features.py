#!/usr/bin/env python3
"""Extract the frozen dynamics, rhythm, and structure features for 30 s clips."""
...
def process(
    row: dict[str, str], audio_dir: Path, beat_dir: Path,
    structure_dir: Path, demix_dir: Path,
) -> dict[str, object]:
    output: dict[str, object] = dict(row)
    nonvocal_paths = [
        demix_dir / "htdemucs" / row["track"] / f"{stem}.wav"
        for stem in ("bass", "drums", "other")
    ]
    if all(path.exists() for path in nonvocal_paths):
        instrumental = np.sum([read_audio(path) for path in nonvocal_paths], axis=0)
        dynamics = dynamics_features(instrumental)
        output["dynamics_signal"] = "demucs_bass_plus_drums_plus_other"
    else:
        dynamics = {feature: float("nan") for feature in DYNAMICS_FEATURES}
    ...
    if beat_path.exists():
        ...
        output["n_beats"] = output["n_downbeats"] = 0
        rhythm = {feature: float("nan") for feature in RHYTHM_FEATURES}
    ...
    if structure_path.exists() and beat_path.exists():
        boundaries = allinone_boundaries(structure_path)
        ...
        structure = {
            feature: float("nan")
            for feature in STRUCTURE_FEATURES + DESCRIPTIVE_STRUCTURE_FEATURES
        }
        output["n_sections"] = 0
    output.update(structure)
    output["structure_eligible"] = int(any(math.isfinite(structure[f]) for f in STRUCTURE_FEATURES))
    return output
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        results = list(executor.map(
            lambda row: process(
                row, args.audio_dir, args.beat_dir, args.structure_dir, args.demix_dir
            ), rows
        ))
