#!/usr/bin/env python3
"""Aggregate independently audited exploratory-v6 results for presentation.

Adapted from v5 presentation aggregation; no existing module or globals are modified.
...
FAMILIES = ("S", "D", "R", "P", "F", "H", "M", "SC")
FAMILY_COUNTS = {"S": 15, "D": 3, "R": 3, "P": 6, "F": 15, "H": 6, "M": 6, "SC": 6}
COMBINATIONS = tuple("+".join(c) for n in range(1, 9) for c in itertools.combinations(FAMILIES, n))
...
BASELINES = tuple(c for c in COMBINATIONS if "SC" not in c.split("+"))
INCREMENTS = tuple((c, c + "+SC") for c in BASELINES)
...
EXPECTED_REAL = {"valid_folds": 38, "primary_fits": 48450, "primary_prediction_rows": 14017350,
                 "diagnostic_fits": 19380, "diagnostic_prediction_rows": 5606940}
ROOT = Path(__file__).resolve().parent.parent
PARENT_PROTOCOL = ROOT / "EXPLORATORY_V6_EVALUATOR_PROTOCOL_EN.md"
PRESENTATION_PROTOCOL = ROOT / "EXPLORATORY_V6_INPUT_PACKAGE_PROTOCOL_EN.md"
AUDITOR = Path(__file__).with_name("audit_equal60_v6_results.py")
AUDITOR_HELPER = Path(__file__).with_name("audit_equal60_v5_results.py")
AUDITOR_HELPER_SHA = "1a928135698d1fce75eea4792696c89b546a914994cc317381c30af1493332fe"
...
def verify_publication(results_dir):
    ...
    require(commit.get("status") == "committed" and commit.get("publication") ==
            "exclusive directory reservation; hardlink COMMIT last", "Uncommitted v6 publication")
    ...
    require({p.name for p in results_dir.iterdir()} == RESULT_FILES | {"COMMIT.json"},
            "Unexpected or missing v6 publication artifact")
    hashes = {}
...
def validate_gate(results_dir, audit_path, audit_sha256, synthetic):
    ...
    require(isinstance(contract, dict) and contract.get("schema_version") == 6
            and contract.get("authorized_stage") == STAGE and contract.get("synthetic_test_only") is synthetic,
            "Frozen v6 contract scope changed")
    require(manifest.get("schema_version") == 6 and manifest.get("stage") == STAGE
            and manifest.get("status") == "completed_exploratory_cv"
            and manifest.get("synthetic_test_only") is synthetic,
            "Run manifest is not a completed v6 evaluation")
    ...
    require(isinstance(accounting, dict), "Missing v6 accounting")
    if synthetic:
        require(type(contract.get("rows")) is int and 0 < contract["rows"] <= 16,
                "Synthetic fixture must contain 1 to 16 marked rows")
        ids = contract.get("population", {}).get("ids", [])
        require(len(ids) == contract["rows"] and len(set(ids)) == len(ids)
                and all(i.startswith("synthetic_v6_") for i in ids), "Explicit synthetic-v6 population required")
    else:
        require(contract.get("rows") == 2174 and contract.get("population", {}).get("label_counts") == {"0": 1278, "1": 896}
                and contract["population"].get("source_rows") == {"Suno":396,"MTG-Jamendo":481,"human_maestro_v3":300,"human_medleydb":156,"human_moisesdb":238,"Mureka_v9":500,"human_saraga_hindustani_v1":103}
                and accounting == EXPECTED_REAL, "Real v6 cohort or accounting changed")
    require({k: len(v) for k, v in contract.get("families", {}).items()} == FAMILY_COUNTS
            and tuple(contract.get("combinations", ())) == COMBINATIONS
    ...
    expected_audit = {"status": "passed", "schema_version": 6, "stage": STAGE,
                      "synthetic_test_only": synthetic, "rows": contract["rows"],
                      "label_counts": contract["population"]["label_counts"], "source_counts": contract["population"]["source_rows"],
                      "families": FAMILY_COUNTS, "candidates": 255,
    ...
    for key, wanted in expected_audit.items():
        require(audit.get(key) == wanted, "Independent v6 audit gate mismatch: " + key)
    package = Path(contract.get("input_package", ""))
    package_commit = package / "COMMIT.json"
    require(package.is_absolute() and package.resolve() == package and package_commit.is_file()
            and not package_commit.is_symlink() and package_commit.resolve() == package_commit,
            "Canonical audited input-package COMMIT required")
    require(sha(package_commit) == audit.get("package_commit_sha256"), "Audited input package changed")
    ...
    hashes[str(package_commit)] = sha(package_commit)
    ...
    require(audit.get("immutable_helper_sha256") == AUDITOR_HELPER_SHA == sha(AUDITOR_HELPER),
            "Independent auditor helper changed")
    hashes[str(AUDITOR.resolve())] = sha(AUDITOR)
    hashes[str(AUDITOR_HELPER.resolve())] = sha(AUDITOR_HELPER)
...
def validate_schedule(contract):
    ...
    require(digest(schedule) == contract.get("schedule_sha256"), "Schedule digest mismatch")
    by_uid, keys = {}, set()
...
def macro_overview(pair_rows):
    return [row for row in pair_rows if row["summary_level"] == "fold_type_macro"]
    ...
    for category in ("primary", "diagnostic"):
        ...
        for table_name in ("pair", "pooled"):
            ...
            for key, values in scopes.items():
                require(key[-1] in expected_modes
                        ...
                        f"Incomplete 255-by-cap {category} {table_name} reporting arm")
        ...
        for key, values in source_scopes.items():
            require(key[2] in expected_modes
                    and {(row["combination"], row["quantity"]) for row in values}
                    == set(itertools.product(COMBINATIONS, expected_caps)),
                    f"Incomplete 255-by-cap {category} source reporting arm")


def sc_increments(pair_rows, source_rows, index_rows):
    """All 127 nonempty old-family subsets, matched with their SC additions."""
    refs = {}
    for r in index_rows:
        key = (r["fold_type"], r["heldout_source"], r["feature_mode"], r["quantity"],
               r["fold_uid"], r["combination"])
        require(key not in refs, "Duplicate paired model identity")
        refs[key] = (r["train_id_set_sha256"], r["test_id_set_sha256"],
                     r["training_rows"], r["test_rows"])
    for baseline, added in INCREMENTS:
        left_keys = [k for k in refs if k[-1] == baseline]
        require(left_keys, "Missing SC baseline: " + baseline)
        for key in left_keys:
            require(refs.get((*key[:-1], added)) == refs[key], "SC pair train/test identities differ")
    identity_digest = digest(sorted(refs.items()))
    output = []
    def append(left, right, baseline, added, scope, metric, endpoint):
        delta = right[metric] - left[metric]
        output.append(dict(scope=scope, summary_level=left.get("summary_level", "held_source_arm"),
            fold_type=left["fold_type"], heldout_source=left["heldout_source"],
            source_group=left.get("source_group", "__two_class__"), endpoint=endpoint,
            feature_mode=left["feature_mode"], quantity=left["quantity"],
            baseline_combination=baseline, added_combination=added,
            baseline_rate=left[metric], added_rate=right[metric],
            added_minus_baseline_pp=100 * delta, direction=_direction(delta),
            paired_identity_registry_sha256=identity_digest))
    pair_lookup = {}
    for r in pair_rows:
        key = tuple(r[k] for k in ("summary_level", "fold_type", "heldout_source",
                                   "feature_mode", "quantity", "combination"))
        require(key not in pair_lookup, "Duplicate pair aggregate")
        pair_lookup[key] = r
    for key, left in pair_lookup.items():
        if key[-1] not in BASELINES:
            continue
        right = pair_lookup.get((*key[:-1], key[-1] + "+SC"))
        require(right is not None, "Missing matched SC pair aggregate")
        require(all(left[k] == right[k] for k in ("held_source_arms", "valid_folds", "metric_cells")),
                "SC pair metric denominators differ")
        for metric in ("roc_auc", "balanced_accuracy"):
            append(left, right, key[-1], key[-1] + "+SC", "two_class_pair_macro", metric, metric)
    source_lookup = {}
    for r in source_rows:
        key = tuple(r[k] for k in ("fold_type", "heldout_source", "feature_mode",
                                   "quantity", "source_group", "combination"))
        require(key not in source_lookup, "Duplicate source aggregate")
        source_lookup[key] = r
    for key, left in source_lookup.items():
        if key[-1] not in BASELINES:
            continue
        right = source_lookup.get((*key[:-1], key[-1] + "+SC"))
        require(right is not None, "Missing matched SC source aggregate")
        require(all(left[k] == right[k] for k in ("label", "endpoint", "unique_oof_recordings",
                                                "global_components", "valid_folds")),
                "SC source endpoint denominators differ")
        for metric in ("recording_rate", "equal_component_rate"):
            append(left, right, key[-1], key[-1] + "+SC", "source_endpoint", metric,
                   left["endpoint"] + "_" + metric)
    require(output, "Empty SC matched deltas")
    return output
...
def render_text(primary_pair, source_rows, ranges, synthetic):
    ...
    lines = ["# Exploratory v6 presentation tables", "", prefix.rstrip(), "" if prefix else "",
             ...
             "## All 255 combinations: macro overview", "",
    ...
    lines.extend(["", "## Interpretation limits", "",
             "SC comparisons pair each of the 127 nonempty original-family subsets with the same subset plus SC on identical training and test IDs. The primary delta CSV retains every cap; diagnostic delta CSV retains both all-cap modes. SC alone is reported among all 255 combinations and has no empty-feature baseline. These are within-v6 comparisons; historical v5 outcome subtraction is not used.", "",
...
def render_latex(primary_pair, synthetic):
    rows = [r for r in primary_pair if r["summary_level"] == "fold_type_macro" ]
    ...
    lines.extend([r"\begin{longtable}{@{}llrrrr@{}}",
                  r"\caption{Exploratory v6 all-combination fold-pair macro overview.}\label{tab:equal60-v6-overview}\\",
...
def export(results_dir, audit_path, audit_sha256, output_dir, synthetic=False):
    ...
    overview = macro_overview(results["primary"]["pair"])
    increments = sc_increments(results["primary"]["pair"], results["primary"]["source"], indexes["primary"][0])
    diagnostic_increments = sc_increments(results["diagnostic"]["pair"], results["diagnostic"]["source"], indexes["diagnostic"][0])
    ...
    try:
        ...
        write_csv(output_dir / "primary_macro_overview_255.csv", overview)
        write_csv(output_dir / "primary_matched_sc_deltas.csv", increments)
        write_csv(output_dir / "diagnostic_all_cap_matched_sc_deltas.csv", diagnostic_increments)
        write_csv(output_dir / "training_row_group_ranges.csv", ranges)
        (output_dir / "EQUAL60_V6_PRESENTATION_TABLES_EN.md").write_text(
            render_text(results["primary"]["pair"], results["primary"]["source"], ranges, synthetic))
        (output_dir / "equal60_v6_presentation_tables_en.tex").write_text(render_latex(results["primary"]["pair"], synthetic))
        ...
        receipt = {"schema_version": 6, "status": "synthetic_fixture_complete" if synthetic else "complete",
                   ...
                   "scope": "Cross-fold descriptive aggregation under the frozen exploratory-v6 estimands. No fitting, scoring, threshold changes, source-arm pooling, candidate selection, or statistical inference.",
                   "primary_family_cap_cells_per_reporting_arm": 1275,
                   "diagnostic_family_cells_per_reporting_arm_and_mode": 255,
                   "all_combinations": list(COMBINATIONS),
                   "matched_sc_pairs": len(INCREMENTS),
                   "matched_sc_increments": [list(x) for x in INCREMENTS],
