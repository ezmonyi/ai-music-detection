#!/usr/bin/env python3
"""Version 2: atomic manifest/shard publication and exclusive writer lock.
...
FHM_CONTRACT_SHA = "7051466edec749af6b2563821eb78cad77b2f444aa8f2b97bfa9dd659e92af1e"
CONFIG = {
...
def publish_bytes(path, body):
    """Publish a complete file atomically; preserve conflicting completed files."""
    path = Path(path)
    if path.exists():
        if path.read_bytes() != body:
            raise ValueError(f"Conflicting frozen publication: {path}")
        return
...
def validate_native(row, reference):
    ...
    if reference.get("extraction_contract_hash") != FHM_CONTRACT_SHA or float(reference.get("requested_duration_sec", 0)) != SECONDS:
        raise ValueError("Reference duration/extraction contract mismatch")
...
def main():
    ...
    writer_lock = (a.output_dir / "writer.lock").open("a")
    ...
    publish_bytes(manifest, body.encode())
    shards = []
