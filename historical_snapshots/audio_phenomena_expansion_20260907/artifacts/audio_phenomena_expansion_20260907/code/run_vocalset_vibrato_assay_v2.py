...
VERSION = "v2-development-only-conflict-safe-before-scoring"
MAX_WORKERS = 4
...
def require_development_authorization(args):
    if args.role != "development" or args.review_decision != "root_approved_development_only":
        raise ValueError("This runner is restricted to root-approved development; no held-out implementation")


def prepare_review_receipt(args: argparse.Namespace) -> int:
    require_development_authorization(args)
    if args.review_receipt.exists():
        raise ValueError("Existing review receipt is immutable; use a new version instead")
    ...
    receipt["runner_scope"] = "development_only_no_heldout_implementation"
...
def run_assay(args: argparse.Namespace) -> int:
    ...
    writer_lock = (args.output_dir / "writer.lock").open("a")
    fcntl.flock(writer_lock.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
    live_inventory = _source_inventory(specs)
    if canonical_hash(live_inventory) != receipt["selected_source_inventory_sha256"]:
        raise ValueError("Live source inventory changed since frozen review")
    contract = {
    ...
    for spec in specs:
        ...
        if receipt_path.exists():
            ...
            if previous.get("contract_hash") != contract_hash:
                raise ValueError("Conflicting prior result contract; existing output preserved")
