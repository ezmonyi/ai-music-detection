...
import prepare_native30_evaluation_inputs_v2 as old

import prepare_native30_evaluation_inputs_v3 as m
...
class AssemblerTests(unittest.TestCase):
    def fixture(self, root, *, missing_sdrp_key=None, schedule_ids=None, recovery=False, recovery_version='extract_native30_sdrp_recovery_v1'):
        root = root.resolve()
        ...
        sdrp_version = recovery_version if recovery else 'extract_native30_sdrp_v1'
        ...
        if recovery:
            ...
            if recovery_version == 'extract_native30_sdrp_operational_v1':
                mapped['beat_provenance'].update(execution_epoch='operational_dispatch_v2', physical_dispatch={'synthetic': True})
    ...
    def test_v3_exact_approved_backend_pin_scope_and_predictor_exclusion(self):
        version = 'extract_native30_sdrp_operational_v1'
        self.assertEqual(m.SDRP_BACKENDS[version]['runner_sha256'],
                         '5e99d91487ff3b2b621101ec06512261d2478fe870564fb44204e50b64964a98')
        previous = dict(m.SDRP_BACKENDS['extract_native30_sdrp_recovery_v1'])
        previous['runner_sha256'] = m.SDRP_BACKENDS[version]['runner_sha256']
        self.assertEqual(m.SDRP_BACKENDS[version], previous)
        with tempfile.TemporaryDirectory() as name:
            root = Path(name).resolve()
            args, _, _, _ = self.fixture(root, recovery=True, recovery_version=version)
            proof = m.publish(m.load_authorities(**args, deep=True), root / 'package')
            self.assertEqual(proof['contract']['version'], m.VERSION)
            self.assertEqual(proof['contract']['sdrp_backend'], version)
            self.assertEqual(set(proof['features'][0]), {'id', *m.FEATURE_NAMES})
            self.assertFalse(any('recovery' in key or 'dispatch' in key or 'epoch' in key or 'beat_provenance' in key
                                 for key in proof['features'][0]))
            source = m.read_json(proof['lineage'][0]['sdrp_receipt']['path'])['payload']
            self.assertEqual(source['row']['beat_provenance']['kind'], 'recovered_serializer_v1')
            with self.assertRaisesRegex(ValueError, 'unapproved S/D/R/P backend'):
                old.load_authorities(**args, deep=True)

    def test_v3_wrong_backend_pin_rejected_not_latest_lookup(self):
        version = 'extract_native30_sdrp_operational_v1'
        with tempfile.TemporaryDirectory() as name:
            args, _, _, _ = self.fixture(Path(name), recovery=True, recovery_version=version)
            changed = copy.deepcopy(m.SDRP_BACKENDS)
            changed[version]['runner_sha256'] = '0' * 64
            with patch.object(m, 'SDRP_BACKENDS', changed):
                with self.assertRaisesRegex(ValueError, 'backend runner pin changed'):
                    m.load_authorities(**args, deep=True)

    def test_synthetic_v2_v3_feature_table_and_metadata_parity(self):
        self.assertEqual(m.digest(Path(old.__file__)),
                         '11f163914c6007e1ba75032da224eed565a06fc33e3cd0cbef8285c5673d3361')
        self.assertEqual(m.FAMILY_CONFIG, old.FAMILY_CONFIG)
        self.assertEqual(m.SCHEDULE_SHA, old.SCHEDULE_SHA)
        self.assertEqual(m.SCOPE, old.SCOPE)
        with tempfile.TemporaryDirectory() as name:
            root = Path(name)
            (root / 'old').mkdir(); (root / 'new').mkdir()
            old_args, _, _, _ = self.fixture(root / 'old', recovery=True)
            new_args, _, _, _ = self.fixture(root / 'new', recovery=True,
                                           recovery_version='extract_native30_sdrp_operational_v1')
            old_rows = old.assemble_rows(old.load_authorities(**old_args, deep=True))
            new_rows = m.assemble_rows(m.load_authorities(**new_args, deep=True))
            self.assertEqual(old_rows[:2], new_rows[:2])
            self.assertEqual(len(new_rows[1][0]), 55)
            self.assertIsNone(new_rows[1][0]['F_phase_residual_cvar_all'])
