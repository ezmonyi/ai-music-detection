    ...
    for shard in contract['shards']:
        ...
        for path in missing:
            ...
            preserve_json(sidecar,dict(status='verified_unavailable_no_beat_events',item_id=row['item_id'],
                ...
                recovery_invocation=[sys.executable,*sys.argv],
                recovery_api='beat_this.inference.File2Beats(checkpoint, device=cuda:gpu, float16=True, dbn=False)(input_path)',
        ...
        preserve_json(receipt_path,dict(status='passed',stage='beats',shard_index=index,run_contract_sha256=contract_sha,
            ...
            original_shard_command=B.command('beats',inputs,runtime,output),outputs={**outputs,**sidecars},
            recovery_invocation=[sys.executable,*sys.argv],
