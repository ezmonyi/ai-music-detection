...
VERSION = 'extract_native30_sdrp_recovery_v1'
FREEZE_VERSION = 'native30-sdrp-recovery-extraction-parent-freeze-v1'
FREEZE_STATUS = 'parent_frozen_for_native30_S_D_R_P_recovery_aware_extraction'
...
def load_recovery():
    return o.load_module(HERE / 'continue_native30_empty_beats_v1.py', RECOVERY_SHA, 'continue_native30_empty_beats_v1')
...
def prepare(freeze_path, freeze_sha):
    ...
    keys = ('runner', 'tests', 'ordinary_extractor', 'recovery_runner', 'recovery_parent_freeze',
            ...
            'origin_plan', 'extractor', 'core', 'bias')
    ...
    expected = {'runner': (HERE / (VERSION + '.py'), None), 'tests': (HERE / ('test_' + VERSION + '.py'), None),
                ...
                'recovery_runner': (HERE / 'continue_native30_empty_beats_v1.py', RECOVERY_SHA),
    ...
    require(continuation['original_parent_freeze'] == bindings['inference_parent_freeze'] and
            ...
            prior['plan'] == bindings['origin_plan'], 'recovery/original/cohort/plan authority join')
...
def verify_completed_inference(contract):
    ...
    return {'verification': proof, 'evidence': evidence, 'beat_provenance': completion['beat_provenance'],
            'recovery_evidence': completion['recovery_evidence'], 'recovered_ids': completion['recovered_ids']}
...
def inspect_inputs(contract, mapped, audited):
    ...
    return {'input': source, 'neural_products': products, 'beat_stage_receipt': binding, 'extractor_input_hashes': paths,
            'beat_provenance': provenance, 'recovery_evidence': record}
