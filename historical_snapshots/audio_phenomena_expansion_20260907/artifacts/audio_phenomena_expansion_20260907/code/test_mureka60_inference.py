...
import prepare_mureka60_inputs as P
import run_mureka60_inference as M
import verify_extract_mureka60 as A
...
def prepared_fixture(root):
    ...
    c = dict(status='frozen_before_materialization', purpose='measurement_only', role=M.ROLE,
             ...
             runtime={}, runtime_sha256=P.canonical_hash({}), rows=src, selected_ids=[r['id'] for r in src], input_file_sha256={})
...
class PreparedTests(unittest.TestCase):
    def setUp(self):
        ...
        for obj,name,value in ((P,'SOURCE',self.source),(P,'OUTPUT',self.out),(P,'audit_inputs',lambda source:(self.src,{}))):
