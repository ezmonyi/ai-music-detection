...
VERSION = 'resume_native30_gpu_guard_v1'
...
HOST = '10-60-169-42'
PLACEMENT = {'host': HOST, 'allinone_logical': [0, 1, 2, 3], 'allinone_physical': [0, 1, 2, 3],
             'beats_logical': [4, 5], 'beats_physical': [6, 5]}
FREEZE_VERSION = 'native30-gpu-guard-operational-parent-freeze-v1'
FREEZE_STATUS = 'parent_frozen_for_native30_gpu_guard_operational_continuation_v1'
...
def stage_evidence(f, prior, effective, stage, shard, rows):
    ...
    return ev, kind, ('preserved_recovery_v2' if historical else 'operational_dispatch_v1'), execution, dispatch
...
def audit_all(f, prior, effective, cohort):
    ...
    for stage in ('allinone', 'beats'):
        for shard in prior['shards']:
            ...
            if epoch == 'operational_dispatch_v1':
