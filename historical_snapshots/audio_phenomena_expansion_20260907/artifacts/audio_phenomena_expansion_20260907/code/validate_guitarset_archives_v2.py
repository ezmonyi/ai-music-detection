...
or pass an external measurement gate. Version 2 adds validated column-oriented
JAMS observations while retaining list-oriented observations and raw source bytes.
"""
...
def _finite_json_value(value):
    """Validate nested namespace-specific JSON values without interpreting them."""
    if value is None or type(value) in (str, bool, int):
        return
    if type(value) is float:
        require(math.isfinite(value), "Nonfinite JAMS observation value")
        return
    ...
    if isinstance(value, dict):
        require(all(isinstance(key, str) for key in value), "Invalid JAMS value object keys")
        for item in value.values():
            _finite_json_value(item)
        return
    raise ValueError("Invalid JAMS observation JSON value")


def _validate_observation(time, duration, value, confidence):
    # Preserve signed finite timestamps: do not repair annotation alignment or
    # impose namespace-specific causal/semantic interpretations.
    require(type(time) in (int, float) and math.isfinite(time),
            "Invalid JAMS observation time")
    require(type(duration) in (int, float) and math.isfinite(duration) and duration >= 0,
            "Invalid JAMS observation duration")
    require(confidence is None or
            (type(confidence) in (int, float) and math.isfinite(confidence)
             and 0 <= confidence <= 1), "Invalid JAMS observation confidence")
    _finite_json_value(value)


def _validated_observation_count(data):
    """Accept exact four-field observation rows or equal-length JSON columns."""
    fields = ("time", "duration", "value", "confidence")
    if isinstance(data, list):
        for observation in data:
            require(isinstance(observation, dict) and set(observation) == set(fields),
                    "Invalid JAMS observation fields")
            _validate_observation(*(observation[field] for field in fields))
        return len(data), "observation_list"
    require(isinstance(data, dict) and set(data) == set(fields),
            "Invalid JAMS annotation data: expected observation list or four columns")
    require(all(isinstance(data[field], list) for field in fields),
            "JAMS column values must be JSON arrays")
    lengths = {len(data[field]) for field in fields}
    require(len(lengths) == 1, "JAMS column arrays have unequal lengths")
    count = len(data["time"])
    for index in range(count):
        _validate_observation(*(data[field][index] for field in fields))
    return count, "column_arrays"


def parse_jams(path):
    """Validate row/column annotation structure without rewriting original bytes.

    Official GuitarSet pitch_contour uses time/duration/value/confidence
    columns; confidence may be null and values may be structured JSON objects.
    This is structural validation, not a complete JAMS namespace-schema check.
    """
    document = strict_json_bytes(regular_file(path).read_bytes())
    require(isinstance(document, dict) and isinstance(document.get("file_metadata"), dict),
            "Invalid JAMS document or file_metadata object")
    duration = document["file_metadata"].get("duration")
    require(type(duration) in (int, float) and math.isfinite(duration) and duration >= 0,
    ...
    for annotation in annotations:
        ...
        count, layout = _validated_observation_count(annotation.get("data"))
        annotation_counts[annotation["namespace"]] += 1
        observation_counts[annotation["namespace"]] += count
    ...
    return {
        ...
        "annotation_data_layout_counts": dict(sorted(layout_counts.items())),
    }
