...
PILOT_COMPARISON = "3b1ea2f43126f2e8fac92b7b9369af5e411345f029d1884932a3c0781e40aa81"
...
def verify_pilot_pcm(wav, pilot_wav, batch_binding, pilot_binding, expected_batch_pcm, expected_pilot_pcm):
    """Require exact waveform identity while binding each container separately."""
    require(a.bind_file(wav, batch_binding["sha256"]) == batch_binding, "batch WAV file binding changed")
    require(a.bind_file(pilot_wav, pilot_binding["sha256"]) == pilot_binding, "pilot WAV file binding changed")
    values = []
    for path, expected in ((wav, expected_batch_pcm), (pilot_wav, expected_pilot_pcm)):
        with sf.SoundFile(path) as stream:
            require((stream.frames, stream.samplerate, stream.channels, stream.format, stream.subtype) ==
                    (a.FRAMES, a.RATE, 2, "WAV", "FLOAT"), "pilot comparison WAV format")
            pcm = stream.read(dtype="float32", always_2d=True)
            require(len(stream.read(1, dtype="float32", always_2d=True)) == 0, "pilot comparison output EOF")
        require(pcm.shape == (a.FRAMES, 2) and np.isfinite(pcm).all(), "pilot comparison shape/finite")
        raw = np.ascontiguousarray(pcm, dtype="<f4").tobytes()
        require(hashlib.sha256(raw).hexdigest() == expected, "pilot comparison receipt PCM hash")
        values.append(raw)
    require(expected_batch_pcm == expected_pilot_pcm and values[0] == values[1], "pilot float32 PCM bit-exact match")
    return {"float32_pcm_sha256": expected_batch_pcm, "float32_bit_exact": True,
            "batch_file_sha256": batch_binding["sha256"], "pilot_file_sha256": pilot_binding["sha256"]}
...
def verify(root, diagnosis_path, diagnosis_sha, run_path, run_sha, pilot, pilot_audit, pilot_comparison):
    ...
    inputs = {"contract": a.bind_file(root / "contract.json", CONTRACT),
              ...
              "pilot_container_pcm_comparison": a.bind_file(pilot_comparison, PILOT_COMPARISON),
              "finalizer": a.bind_file(Path(__file__).resolve()),
              "finalizer_tests": a.bind_file(Path(__file__).resolve().with_name("test_finalize_native30_partial_intake_v3.py")),
    ...
    comparison = a.load(pilot_comparison)
    comparison_records = indexed(comparison.get("records"), "pilot container/PCM comparison")
    require(comparison.get("status") == "eight_pilot_container_and_pcm_comparison_completed" and
            comparison.get("count") == comparison.get("pcm_bit_exact_count") == 8 and
            set(comparison_records) == set(pilot_wavs), "eight pilot comparison receipt status/IDs")
    require(set(pilot_wavs) <= success_ids, "all eight pilot IDs must be eligible")
    pilot_bindings, pilot_receipts = {}, {}
    for ident, (name, value) in pilot_wavs.items():
        ...
        item_name = "items/" + ident + ".json"
        require(item_name in pc["products"], "pilot receipt must be committed")
        item_binding = a.bind_file(pilot / item_name, pc["products"][item_name]["sha256"])
        require(item_binding["signature"][2] == pc["products"][item_name]["bytes"], "pilot receipt byte count")
        upstream[str(pilot / item_name)] = item_binding
        pilot_receipts[ident] = a.load(pilot / item_name)
        require(pilot_receipts[ident].get("id") == ident and
                pilot_receipts[ident].get("file_sha256") == binding["sha256"], "pilot receipt/WAV identity")
    ...
    for index, ident in enumerate(sorted(success_ids), 1):
        ...
        if ident in pilot_wavs:
            require(replay[ident]["float32_bit_exact"] is True, "pilot independent replay must pass")
            prior = pilot_receipts[ident]
            expected_pcm = prior["audit"]["output_waveform_float32_sha256"]
            observation = comparison_records[ident]
            require(observation.get("pcm_bit_exact") is True and observation.get("max_abs_sample_error") == 0 and
                    observation.get("batch_wav", {}).get("sha256") == products[str(wav)]["sha256"] and
                    observation.get("pilot_wav", {}).get("sha256") == pilot_bindings[ident]["sha256"] and
                    observation.get("batch_float32_pcm_sha256") == receipt["waveform_float32_sha256"] and
                    observation.get("pilot_float32_pcm_sha256") == expected_pcm, "pilot comparison receipt current-file/PCM join")
            verify_pilot_pcm(wav, Path(pilot_bindings[ident]["path"]), products[str(wav)], pilot_bindings[ident],
                             receipt["waveform_float32_sha256"], expected_pcm)
    ...
    manifest = {"version": "finalize_native30_partial_intake_v3", "status": STATUS, "contract_sha256": CONTRACT,
                "original_root": str(root), "original_run_status": "partial_no_COMMIT", "run_summary_sha256": run_sha,
                "diagnosis_sha256": diagnosis_sha, "pilot_container_pcm_comparison_sha256": PILOT_COMPARISON, "policy": POLICY, "attempted": TOTAL, "eligible_count": ELIGIBLE,
    ...
    summary = {"status": STATUS, "metadata_candidates": TOTAL + MONO, "attempted": TOTAL, "eligible": ELIGIBLE,
               ...
               "pilot_container_pcm_comparison_sha256": PILOT_COMPARISON,
...
def finalize(root, diagnosis_path, diagnosis_sha, run_path, run_sha, pilot, pilot_audit, pilot_comparison, output):
    root, diagnosis_path, run_path, pilot, pilot_audit, pilot_comparison, output = [Path(p).absolute() for p in
        (root, diagnosis_path, run_path, pilot, pilot_audit, pilot_comparison, output)]
    ...
    with lock.open("rb") as stream:
        ...
        manifest, summary, bindings, successes, failures = verify(root, diagnosis_path, diagnosis_sha, run_path, run_sha, pilot, pilot_audit, pilot_comparison)
        ...
        inventory(root, successes, failures)
        commit = {"status": STATUS, "version": "finalize_native30_partial_intake_v3", "contract_sha256": CONTRACT,
...
def main():
    ...
    for name in ("root", "diagnosis", "diagnosis-sha256", "run-summary", "run-summary-sha256", "pilot", "pilot-audit", "pilot-comparison", "output"):
        parser.add_argument("--" + name, required=True)
    args = parser.parse_args()
    result = finalize(args.root, args.diagnosis, args.diagnosis_sha256, args.run_summary, args.run_summary_sha256,
                      args.pilot, args.pilot_audit, args.pilot_comparison, args.output)
