...
def main():
    ...
    ap.add_argument("--output", type=Path, default=art / "audit/native30_evidence_review_v1.json")
    ...
    report = {
        "schema_version": "native30-evidence-review-v1",
        ...
        "implementation_binding": {"path": str(Path(__file__).resolve()), "sha256": sha256(Path(__file__).resolve())},
        "counts": {
        ...
        "replay_command": "python artifacts/audio_phenomena_expansion_20260907/code/probe_native30_evidence_v1.py --probe-headers --output <new-json-path>",
        "remaining_boundaries": [
            ...
            "FMA/Suno registry exact30 paths are under demucs_bias_corrected_1000_20260901 and lack a bound derivative SHA256/creation receipt in the inspected inputs; no equivalence to a native full mix or simple resampling is assumed.",
            "The preferred future treatment for the 397 native-stereo FMA/Suno rows is one uniform new exact30 native-origin preprocessing rule without Demucs/spectral correction, unless the old transformation is first proven equivalent and uniformly admissible.",
            "Applying the existing spectral/Demucs correction only to selected sources would create unequal preprocessing and is not supported.",
    ...
    # Detect input drift across the potentially slower remote-header phase.
    for name, path in inputs.items():
