...
\subsection{Document build and incomplete final bindings}
The working master is
\path{artifacts/audio_phenomena_expansion_20260907/latex/interpretable_audio_thesis_final_working_en.tex}.
From its containing directory, the retained build command is
\begin{verbatim}
tectonic --keep-logs --outdir ../output/pdf \
  interpretable_audio_thesis_final_working_en.tex
\end{verbatim}
...
every page. This working appendix does not yet supply terminal Native30
classification/BC-increment hashes because those results remain unfinished.
Their actual verified bindings, runtime manifests and commands must be added
before the document can be designated the final thesis. No placeholder hash
or prospective fit count is substituted for completion evidence.\par
\endgroup
