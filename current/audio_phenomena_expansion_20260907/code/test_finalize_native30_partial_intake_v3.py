...
import finalize_native30_partial_intake_v3 as f
...
def fixture(base):
    ...
    comparison_path = base / "pilot_comparison.json"
    comparison_records = []
    for receipt in receipts[:8]:
        ident = receipt["id"]
        pilot_item = {"id": ident, "file_sha256": receipt["file_sha256"], "audit": receipt["audit"]}
        item_path = pilot / "items" / (ident + ".json")
        item_path.parent.mkdir(exist_ok=True)
        item_sha = write(item_path, pilot_item)
        pilot_products["items/" + ident + ".json"] = {"sha256": item_sha, "bytes": item_path.stat().st_size}
        comparison_records.append({"id": ident, "pcm_bit_exact": True, "max_abs_sample_error": 0,
                                   "batch_wav": {"sha256": receipt["file_sha256"]},
                                   "pilot_wav": {"sha256": receipt["file_sha256"]},
                                   "batch_float32_pcm_sha256": receipt["waveform_float32_sha256"],
                                   "pilot_float32_pcm_sha256": receipt["waveform_float32_sha256"]})
    pc_sha = write(pilot / "COMMIT.json", {"status": "committed_DSP_pilot_only", "products": pilot_products})
    audit_value = f.a.load(pa); audit_value["bindings"]["commit"] = pc_sha; pa_sha = write(pa, audit_value)
    comparison_sha = write(comparison_path, {"status": "eight_pilot_container_and_pcm_comparison_completed",
                                            "count": 8, "pcm_bit_exact_count": 8, "records": comparison_records})
    return {"contract": contract, "run": run, "diagnosis": diagnosis, "contract_sha": contract_sha,
            "pc_sha": pc_sha, "pa_sha": pa_sha, "comparison_sha": comparison_sha,
            "args": (root, dp, ds, run_path, run_sha, pilot, pa, comparison_path),
...
class SyntheticTests(unittest.TestCase):
    def setUp(self):
        ...
        for target, name, value in ((f, "CONTRACT", self.data["contract_sha"]),
                                    (f.a, "PILOT_COMMIT", self.data["pc_sha"]), (f.a, "PILOT_AUDIT", self.data["pa_sha"]), (f, "PILOT_COMPARISON", self.data["comparison_sha"])):
            p = patch.object(target, name, value); p.start(); self.addCleanup(p.stop)
    ...
    def test_relocation_preserves_original_path_and_expected_hash(self):
        local, remote = self.base / "declared_artifacts", self.base / "remote_artifacts"
        remote.mkdir(); (remote / "audit").mkdir()
        copied = remote / "audit" / "receipt.json"
        copied.write_bytes(b"same immutable artifact")
        declared = str(local / "audit" / "receipt.json")
        bindings = {}
        with patch.object(f, "LOCAL_ARTIFACT_ROOT", local), patch.object(f, "REMOTE_ARTIFACT_ROOT", remote):
            f.bind_declared_files({"path": declared, "sha256": f.a.sha(copied), "bytes": copied.stat().st_size}, bindings)
            self.assertEqual(set(bindings), {declared})
            self.assertEqual(bindings[declared]["path"], str(copied))
            f.a.recheck(bindings)
            with self.assertRaisesRegex(ValueError, "SHA mismatch"):
                f.bind_declared_files({"path": declared, "sha256": "0" * 64}, {})
            copied.write_bytes(b"mutated")
            with self.assertRaises(ValueError): f.a.recheck(bindings)

    def test_relocation_rejects_missing_lookalike_and_traversal_paths(self):
        local, remote = self.base / "declared_artifacts", self.base / "remote_artifacts"
        remote.mkdir()
        with patch.object(f, "LOCAL_ARTIFACT_ROOT", local), patch.object(f, "REMOTE_ARTIFACT_ROOT", remote):
            for path in (str(local / "missing.json"), str(self.base / "declared_artifacts_extra" / "missing.json"),
                         str(self.base / "unrelated" / "missing.json"), "relative.json", str(local) + "/../escape.json"):
                with self.subTest(path=path), self.assertRaises(ValueError):
                    f.bind_declared_files({"path": path, "sha256": "a" * 64}, {})
            external = self.base / "external.json"; external.write_bytes(b"external")
            (remote / "redirect.json").symlink_to(external)
            with self.assertRaises(ValueError):
                f.bind_declared_files({"path": str(local / "redirect.json"), "sha256": f.a.sha(external)}, {})

    def test_historical_relocation_is_exact_file_and_digest_allowlist(self):
        declared = "/historical/project/one.json"
        copied = self.base / "historical.json"; copied.write_bytes(b"pinned metadata")
        digest = f.a.sha(copied)
        mapping = {declared: {"path": str(copied), "sha256": digest}}
        with patch.object(f, "HISTORICAL_METADATA_COPIES", mapping):
            bindings = {}; f.bind_declared_files({"path": declared, "sha256": digest}, bindings)
            self.assertEqual(bindings[declared]["path"], str(copied))
            with self.assertRaisesRegex(ValueError, "allowlist SHA mismatch"):
                f.bind_declared_files({"path": declared, "sha256": "f" * 64}, {})
            with self.assertRaises(ValueError):
                f.bind_declared_files({"path": "/historical/project/two.json", "sha256": digest}, {})
            copied.write_bytes(b"changed metadata")
            with self.assertRaises(ValueError):
                f.bind_declared_files({"path": declared, "sha256": digest}, {})

    def test_different_peak_timestamps_accept_exact_pcm_and_bind_each_file(self):
        wav = self.data["args"][0] / "audio" / "id0.wav"
        reference = self.base / "reference_different_timestamp.wav"
        raw = bytearray(wav.read_bytes())
        offset = 12; found = False
        while offset < len(raw):
            length = struct.unpack_from("<I", raw, offset + 4)[0]
            if raw[offset:offset + 4] == b"PEAK":
                timestamp = struct.unpack_from("<I", raw, offset + 12)[0]
                struct.pack_into("<I", raw, offset + 12, (timestamp + 1) % (2 ** 32))
                found = True; break
            offset += 8 + length + (length & 1)
        self.assertTrue(found, "synthetic float32 WAV must include valid PEAK timestamp")
        reference.write_bytes(raw)
        batch_binding, pilot_binding = f.a.bind_file(wav), f.a.bind_file(reference)
        self.assertNotEqual(batch_binding["sha256"], pilot_binding["sha256"])
        values, _ = sf.read(wav, dtype="float32", always_2d=True)
        pcm = hashlib.sha256(np.ascontiguousarray(values, dtype="<f4").tobytes()).hexdigest()
        result = f.verify_pilot_pcm(wav, reference, batch_binding, pilot_binding, pcm, pcm)
        self.assertTrue(result["float32_bit_exact"])
        self.assertEqual(result["batch_file_sha256"], batch_binding["sha256"])
        self.assertEqual(result["pilot_file_sha256"], pilot_binding["sha256"])
        wrong = dict(pilot_binding); wrong["sha256"] = batch_binding["sha256"]
        with self.assertRaises(ValueError): f.verify_pilot_pcm(wav, reference, batch_binding, wrong, pcm, pcm)

    def test_one_sample_mutation_rejected_even_with_valid_individual_file_hashes(self):
        wav = self.data["args"][0] / "audio" / "id0.wav"
        reference = self.base / "reference_changed_sample.wav"
        values, _ = sf.read(wav, dtype="float32", always_2d=True)
        original_pcm = hashlib.sha256(np.ascontiguousarray(values, dtype="<f4").tobytes()).hexdigest()
        changed = values.copy(); changed[1, 0] = np.nextafter(changed[1, 0], np.float32(1))
        self.assertEqual(np.count_nonzero(values != changed), 1)
        sf.write(reference, changed, f.a.RATE, format="WAV", subtype="FLOAT")
        changed_pcm = hashlib.sha256(np.ascontiguousarray(changed, dtype="<f4").tobytes()).hexdigest()
        with self.assertRaisesRegex(ValueError, "PCM bit-exact match"):
            f.verify_pilot_pcm(wav, reference, f.a.bind_file(wav), f.a.bind_file(reference), original_pcm, changed_pcm)
