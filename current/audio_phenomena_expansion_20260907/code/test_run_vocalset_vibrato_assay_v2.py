...
from run_vocalset_vibrato_assay_v2 import (
...
class VocalSetVibratoRunnerTest(unittest.TestCase):
    ...
    def test_heldout_runner_is_disabled(self):
        from types import SimpleNamespace
        for role, decision in (("evaluation", "root_approved_heldout_evaluation"), ("development", "unreviewed")):
            with self.assertRaisesRegex(ValueError, "restricted to root-approved development"):
                require_development_authorization(SimpleNamespace(role=role, review_decision=decision))
        require_development_authorization(SimpleNamespace(role="development", review_decision="root_approved_development_only"))
