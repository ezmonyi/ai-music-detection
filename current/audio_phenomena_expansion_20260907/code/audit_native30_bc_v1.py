...
def _runtime_file_binding(entry, label):
    """Validate one frozen runtime file binding against the file it names.

    Runtime manifests store the complete binding dictionary.  The filesystem
    helper therefore receives the binding's ``path`` field, while the full
    dictionary is compared to the freshly computed result.
    """
    require(isinstance(entry, dict) and set(entry) == {"path", "bytes", "sha256"},
            label + " shape")
    require(file_binding(entry["path"]) == entry, label)
...
def _validate_runtime(runtime):
    ...
    # Pass each binding's path to the filesystem helper and compare the full
    # path/size/hash tuple against the bytes currently on disk.
    module_files = runtime.get("module_files", {})
    package_files = runtime.get("package_files", {})
    require(isinstance(module_files, dict), "runtime module files manifest")
    require(isinstance(package_files, dict), "runtime package files manifest")
    for name, entry in module_files.items():
        _runtime_file_binding(entry, "runtime module binding: " + str(name))
    for name, entry in package_files.items():
        _runtime_file_binding(entry, "runtime package binding: " + str(name))
    if runtime.get("libsndfile_binary"):
        _runtime_file_binding(runtime["libsndfile_binary"], "libsndfile binary pin")
