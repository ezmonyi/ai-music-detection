...
class OperationalTests(unittest.TestCase):
    ...
    def test_only_explicit_remap_and_disjoint_workers(self):
        ...
        self.assertEqual(self.effective['aio_gpus'], [7]); self.assertEqual(self.prior['aio_gpus'], [0, 1, 2, 3])
        shards = [{'index': i} for i in range(160)]
        for stage, gpus in (('allinone', [7]), ('beats', [6, 5])):
            assignments = list(s.b.assignments(shards, gpus))
            assigned = [shard['index'] for _, group in assignments for shard in group]
            self.assertEqual(sorted(assigned), list(range(160))); self.assertEqual(len(set(assigned)), 160)
            self.assertEqual(len(assignments), len(gpus))
            for gpu, group in assignments:
                self.assertTrue(all(s.v2.assigned_gpu(self.effective, stage, shard['index']) == gpu for shard in group))
    ...
    def test_new_attempt_records_actual_gpu6_and_separate_old_intent(self):
        ...
        self.assertEqual(epoch, 'operational_dispatch_v2'); self.assertEqual(s.b.digest(old), old_hash)
    ...
    def test_combined_graph_and_exhaustive_inventory(self):
        ...
        self.assertTrue(all(v['execution_epoch'] == 'operational_dispatch_v2' for v in provenance.values()))
