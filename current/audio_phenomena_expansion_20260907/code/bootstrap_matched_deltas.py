...
def _evaluator_contract_hash(contract: dict[str, Any]) -> str:
    """Match the evaluator's canonical JSON hashing convention exactly."""
    payload = json.dumps(
...
def verify_completed_evaluation_authorization(manifest: dict[str, Any]) -> dict[str, Any]:
    """Verify both the run declaration and the independently frozen receipt."""
    contract = manifest.get("contract")
    authorization = manifest.get("authorization")
    if not isinstance(contract, dict) or not isinstance(authorization, dict):
        raise ValueError("Evaluation manifest lacks contract/authorization objects")
    if authorization.get("status") != "frozen_verified":
        raise ValueError("Development evaluation authorization must be frozen_verified")
    contract_hash = _evaluator_contract_hash(contract)
    if authorization.get("contract_sha256") != contract_hash:
        raise ValueError("Authorization contract hash does not match the manifest contract")
    receipt_value = authorization.get("receipt_path")
    receipt_sha256 = authorization.get("receipt_sha256")
    if not isinstance(receipt_value, str) or not receipt_value:
        raise ValueError("Authorization does not declare the frozen receipt path")
    if not isinstance(receipt_sha256, str) or len(receipt_sha256) != 64:
        raise ValueError("Authorization does not declare a valid receipt SHA-256")
    receipt_path = Path(receipt_value).resolve()
    if not receipt_path.is_file():
        raise FileNotFoundError(f"Declared frozen receipt is unavailable: {receipt_path}")
    actual_receipt_sha256 = sha256_file(receipt_path)
    if actual_receipt_sha256 != receipt_sha256:
        raise ValueError("Frozen receipt bytes differ from the authorization receipt SHA-256")
    receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
    if receipt.get("status") != "frozen":
        raise ValueError("Declared receipt itself is not frozen")
    if receipt.get("authorized_stage") != "dev" or contract.get("authorized_stage") != "dev":
        raise ValueError("Frozen receipt and contract must both authorize development evaluation")
    if receipt.get("contract_sha256") != contract_hash:
        raise ValueError("Frozen receipt contract hash does not match the manifest contract")
    receipt_contract = receipt.get("contract")
    if not isinstance(receipt_contract, dict):
        raise ValueError("Frozen receipt does not embed its exact contract")
    if receipt_contract != contract or _evaluator_contract_hash(receipt_contract) != contract_hash:
        raise ValueError("Frozen receipt embeds a different evaluation contract")
    return {
...
def load_inputs(evaluation_dir: Path) -> dict[str, Any]:
    ...
    verified_authorization = verify_completed_evaluation_authorization(manifest)
    ...
    return {
        "manifest": manifest,
        "paths": (*inputs, verified_authorization["receipt_path"]),
