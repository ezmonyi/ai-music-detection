#!/usr/bin/env python3
"""Corrected independent reserved BC replay; separate correction authority required.
...
VERSION = 'audit_bc_guitarset_reserved_admission_v2'
ORIGINAL_AUDITOR_SHA = 'cba9e6821e96f5280b21a724559a5b78c43e5b8a1995b8a72e69f3089f6893b7'
ORIGINAL_FREEZE_SHA = '121b478a41ce45857d915ef5854771599532d6c5d2e9104bb79ebde574d3f253'
ORIGINAL_COMMIT_SHA = 'a96b7ecc93f4bd0c44954b6c64afc0a9b4c782482ec025291a248dd35862b2e4'
ORIGINAL_FAILED_LOG_SHA = '369434507413494bd75939a83cd2633f491d9cd6ac93dc8d6543bf6df70e2f9c'
CORRECTION_VERSION = 'bc_reserved_independent_audit_correction_authority_v1'
CORRECTION_SCOPE = {'independent_reserved_reaudit_authorized': True, 'source_or_product_modification_authorized': False,
    'producer_rerun_authorized': False, 'tolerance_changes_authorized': False, 'threshold_changes_authorized': False,
    'BC_admission_authorized': False, 'classifier_fits_authorized': False, 'model_scoring_authorized': False}
CORRECTION_DESCRIPTION = 'independent_normalized_columns_explicit_F_order_and_contiguous_assertion_only'
DIAGNOSIS_PINS = {
    'audit/bc_reserved_raw_sum_diagnosis_v1/DIAGNOSIS_EN.md': '020e0b2853eb1fb61fadaa9bf8c4208b03a9435766ba79c744e9123089da7fe6',
    'audit/bc_reserved_raw_sum_diagnosis_v1/diagnose.py': '30a4b8801fa6e250430b4bbd479e6318416486746a33876c73f2bd75ac1d1c5d',
    'audit/bc_reserved_raw_sum_diagnosis_v1/result.json': 'f332406861511da5efba1430601df07a2ede34b1f72d32812232217fa2a4f206',
    'audit/bc_reserved_raw_sum_diagnosis_v1/capture_exception_v2.py': '6ab292f0e9d86d58dfb32005a97fdcb5891ab0e4c024cb5ab372e9b3885992f6',
    'audit/bc_reserved_raw_sum_diagnosis_v1/capture_process01/report.json': 'f3d3c6d3d458ff29a41dafad1b8575d3c45336c1dc101f6082613eadba2e7046',
    'audit/bc_reserved_raw_sum_diagnosis_v1/capture_process01/captured_arrays.npz': '3889c43131d9e5a6a0028d1b9406a5fd6af9017248835f8c8fa64d26fb60b57b',
    'audit/bc_reserved_raw_sum_diagnosis_v1/probe_alignment_v3.py': '22168c154d81d73524107817e3218c1d63dae3cfca8b53c6c5b150ac4500b6aa',
    'audit/bc_reserved_raw_sum_diagnosis_v1/probe_neighbor_v4.py': '3456b08ede577c8df06583ade90611a7e953642889348c25c81dc1cb3f0db9e2',
    'audit/bc_reserved_raw_sum_diagnosis_v1/neighbor_process01.json': '7ba683f2ddccc5adfb9ca77a940fed6aceecf430ea7159b37cd78ae1c5c39724',
    'audit/bc_reserved_raw_sum_diagnosis_v1/parent_decimal_check.py': '19845a44060e67c29ccf173d17408f1dae12b9587269a814b0e1a3410e04e880',
    'audit/bc_reserved_raw_sum_diagnosis_v1/parent_decimal_result.json': 'da6b105db0d542ee07f79ce49f4ead00ee50c252b08392f9a3572fd26d0fb233'}
...
def primitive(coefficients, bins):
    ...
    normalized = np.empty(columns.shape, np.complex128, order='F')
    require(normalized.flags.f_contiguous and normalized.strides[0] == normalized.itemsize,
            'corrected normalized columns must be contiguous')
...
def verify_correction_authority(path, expected_sha, *, rc, original_freeze, original_commit, original_auditor, own, tests):
    """A separate authority may authorize v2 replay; the original freeze cannot."""
    require(path is not None and expected_sha is not None, 'separate correction authority is mandatory')
    correction, entry = read_json(path, expected_sha)
    require(original_freeze['sha256'] == ORIGINAL_FREEZE_SHA and original_commit['sha256'] == ORIGINAL_COMMIT_SHA and
        original_auditor['sha256'] == ORIGINAL_AUDITOR_SHA, 'correction exact original authority pins')
    original_log = binding(rc / 'logs/bc_reserved_independent_actual_v1.log', ORIGINAL_FAILED_LOG_SHA)
    diagnosis = {relative: binding(rc / relative, expected) for relative, expected in DIAGNOSIS_PINS.items()}
    expected = {'version': CORRECTION_VERSION, 'status': 'authorized_independent_auditor_v2_replay_only_not_admission',
        'original_parent_freeze': original_freeze, 'original_result_COMMIT': original_commit, 'original_auditor': original_auditor,
        'original_failed_audit_log': original_log, 'corrected_auditor': own, 'corrected_tests': tests,
        'diagnosis': diagnosis, 'scope': CORRECTION_SCOPE, 'correction': CORRECTION_DESCRIPTION}
    require(correction == expected, 'exact separate correction authority identity/scope/diagnosis')
    a.check_file_bindings(correction)
    return {'correction_authority': entry, 'original_auditor': original_auditor, 'original_failed_audit_log': original_log,
            'correction_diagnosis': diagnosis, 'correction': CORRECTION_DESCRIPTION}


def validate_authorities(root, commit_sha, draft_dir, freeze_path, freeze_sha, self_sha, tests_path, tests_sha,
                         correction_path=None, correction_sha=None):
    forbid_producer_imports()
    require(correction_path is not None and correction_sha is not None, 'separate correction authority is mandatory')
    own = binding(Path(__file__).resolve(), self_sha); tests = binding(tests_path, tests_sha)
    original_auditor = binding(Path(__file__).resolve().with_name('audit_bc_guitarset_reserved_admission_v1.py'), ORIGINAL_AUDITOR_SHA)
    ...
    require(freeze.get('version') == 'bc_guitarset_reserved_admission_parent_freeze_v1' and freeze.get('status') == 'authorized_reserved_measurement_not_admission' and
        ...
        freeze.get('independent_auditor') == original_auditor and freeze.get('codec_recipes') == document['codec_recipes'] and
    ...
    require(commit.get('version') == PRODUCER_VERSION and commit.get('status') in (
        ...
        commit.get('parent_freeze') == freeze_entry and commit.get('independent_auditor') == original_auditor and commit.get('expected_measurements') == EXPECTED,
        'terminal measurement COMMIT authority/scope')
    correction_proof = verify_correction_authority(correction_path, correction_sha, rc=Path(__file__).resolve().parent.parent,
        original_freeze=freeze_entry, original_commit=commit_entry, original_auditor=original_auditor, own=own, tests=tests)
    ...
    return document, freeze, commit, inventory, {'audit_code': own, 'audit_tests': tests, 'draft': draft, **correction_proof,
...
def audit(root, commit_sha, draft_dir, freeze_path, freeze_sha, receipt_path, self_sha, tests_path, tests_sha,
    ...
    document, freeze, commit, inventory, upstream = validate_authorities(root, commit_sha, draft_dir, freeze_path, freeze_sha, self_sha, tests_path, tests_sha, correction_path, correction_sha)
    ...
    end = validate_authorities(root, commit_sha, draft_dir, freeze_path, freeze_sha, self_sha, tests_path, tests_sha, correction_path, correction_sha)
...
def main():
    ...
    for name in ('result', 'result-commit-sha256', 'draft-dir', 'parent-freeze', 'parent-freeze-sha256', 'correction-authority', 'correction-authority-sha256',
    ...
    result = audit(x.result, x.result_commit_sha256, x.draft_dir, x.parent_freeze, x.parent_freeze_sha256,
                   x.output_receipt, x.audit_code_sha256, x.audit_tests, x.audit_tests_sha256, x.correction_authority, x.correction_authority_sha256)
