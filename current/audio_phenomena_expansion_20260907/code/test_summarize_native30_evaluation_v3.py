...
import summarize_native30_evaluation_v3 as m
import summarize_native30_evaluation_v2 as old
...
def synthetic_publication(directory):
    ...
    code_bindings = {name: m.binding(m.HERE / name) for name in ('evaluate_native30_v3.py', 'test_evaluate_native30_v3.py')}
    contract = {'version': 'evaluate_native30_v3', 'stage': 'native30_seven_family_schedule_only_development_evaluation_v3',
    ...
    write(freeze_path, {'version': 'native30-evaluation-parent-freeze-v3', 'status': 'parent_frozen_for_native30_fold_only_fitting',
    ...
    write(root / 'manifest.json', {'version': 'evaluate_native30_v3', 'contract_sha256': contract_sha, 'model_instances': len(valid),
    ...
    write(root / 'COMMIT.json', {'version': 'evaluate_native30_v3', 'status': 'committed_fold_only_evaluation_not_model_selection',
...
class PublicationTests(unittest.TestCase):
    ...
    def test_v3_report_numerical_and_english_markdown_parity_with_v2(self):
        self.assertEqual(m.binding(Path(old.__file__))['sha256'],
                         '625c36ea23f1ba85c1197881fad762d082b59d4874e1c580e2925e6cf9a0bd30')
    ...
    def test_v3_exact_evaluator_code_and_test_pins_required(self):
        self.assertEqual(m.binding(m.HERE / 'evaluate_native30_v3.py')['sha256'], m.EVALUATOR_SHA)
        self.assertEqual(m.binding(m.HERE / 'test_evaluate_native30_v3.py')['sha256'], m.EVALUATOR_TEST_SHA)
