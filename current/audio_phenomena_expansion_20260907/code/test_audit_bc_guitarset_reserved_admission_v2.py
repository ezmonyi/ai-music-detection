...
import audit_bc_guitarset_reserved_admission_v2 as r
...
class NumericsTests(unittest.TestCase):
    def test_controlled_adjacency_old_scalar_vs_new_contiguous_layout(self):
        # Synthetic operands only. On the pinned x86/FMA runtime, the old C layout
        # changes rounding under a conservative false overlap; F columns do not.
        features = np.core._multiarray_umath.__cpu_features__
        if not features.get('FMA3'):
            self.skipTest('specific old scalar/FMA dispatch regression requires pinned x86 FMA3 runtime')
        rng = np.random.Generator(np.random.PCG64(7061))
        values = (rng.uniform(-1,1,(247,3)) + 1j*rng.uniform(-1,1,(247,3))).astype(np.complex128)
        reference = None; failing = None
        for order in ('C','F'):
            for gap in (0,16,32,48,64,128,4096):
                backing = np.empty(values.nbytes+gap+247*16+64,np.uint8)
                offset = (-backing.ctypes.data)%64
                normalized = np.ndarray(values.shape,np.complex128,buffer=backing,offset=offset,order=order)
                product = np.ndarray((247,),np.complex128,buffer=backing,offset=offset+values.nbytes+gap)
                normalized[:] = values
                self.assertFalse(np.shares_memory(normalized[:,0],product)); self.assertFalse(np.shares_memory(normalized[:,1],product))
                np.multiply(normalized[:,0],normalized[:,1],out=product)
                if order=='C' and gap==0: failing=product.copy()
                if order=='C' and gap==32: reference=product.copy()
                if order=='C' and gap<=16: np.testing.assert_array_equal(product,failing)
                elif reference is not None: r.exact_array(product,reference,'contiguous corrected product independent of adjacent gap')
        self.assertTrue(np.any(failing!=reference))
        explicit = np.empty(247,np.complex128)
        explicit.real=values[:,0].real*values[:,1].real-values[:,0].imag*values[:,1].imag
        explicit.imag=values[:,0].real*values[:,1].imag+values[:,0].imag*values[:,1].real
        r.exact_array(failing,explicit,'old path is separate rounding')
        # Exercise the actual v2 allocator and its invariant, not just a model.
        coefficients = np.zeros((247,513),np.complex128); coefficients[:,[32,48,80]]=values
        seen=[]; allocate=np.empty
        def recording_empty(shape,dtype=float,**kwargs):
            value=allocate(shape,dtype,**kwargs)
            if tuple(np.atleast_1d(shape))==(247,3): seen.append((kwargs.get('order'),value.strides,value.flags.f_contiguous))
            return value
        with patch.object(r.np,'empty',side_effect=recording_empty): r.primitive(coefficients,[32,48,80])
        self.assertIn(('F',(16,3952),True),seen)
...
from test_audit_bc_guitarset_reserved_admission_v2 import summary_fixture
...
class CorrectionAuthorityTests(unittest.TestCase):
    def test_missing_correction_rejects_before_authority_or_source_access(self):
        with patch.object(r,'read_json') as read:
            with self.assertRaisesRegex(ValueError,'mandatory'):
                r.validate_authorities(Path('/synthetic/result'),'commit',Path('/synthetic/draft'),Path('/synthetic/freeze'),'freeze','self',Path('/synthetic/tests'),'tests')
            read.assert_not_called()

    def test_exact_separate_authority_and_mutation_rejections(self):
        with tempfile.TemporaryDirectory() as tmp, contextlib.ExitStack() as stack:
            root=Path(tmp); (root/'logs').mkdir()
            paths={}
            for name in ('freeze','commit','original','own','tests','diagnosis'):
                path=root/name; path.write_bytes(('synthetic_'+name).encode()); paths[name]=r.binding(path)
            log=root/'logs/bc_reserved_independent_actual_v1.log'; log.write_text('synthetic failed original audit retained')
            for name,value in [('ORIGINAL_FREEZE_SHA',paths['freeze']['sha256']),('ORIGINAL_COMMIT_SHA',paths['commit']['sha256']),
                ('ORIGINAL_AUDITOR_SHA',paths['original']['sha256']),('ORIGINAL_FAILED_LOG_SHA',r.digest(log)),
                ('DIAGNOSIS_PINS',{'diagnosis':paths['diagnosis']['sha256']})]: stack.enter_context(patch.object(r,name,value))
            authority={'version':r.CORRECTION_VERSION,'status':'authorized_independent_auditor_v2_replay_only_not_admission',
                'original_parent_freeze':paths['freeze'],'original_result_COMMIT':paths['commit'],'original_auditor':paths['original'],
                'original_failed_audit_log':r.binding(log),'corrected_auditor':paths['own'],'corrected_tests':paths['tests'],
                'diagnosis':{'diagnosis':paths['diagnosis']},'scope':r.CORRECTION_SCOPE,'correction':r.CORRECTION_DESCRIPTION}
            path=root/'correction.json'
            def call(value=authority):
                path.write_bytes(r.canonical(value)); return r.verify_correction_authority(path,r.digest(path),rc=root,
                    original_freeze=paths['freeze'],original_commit=paths['commit'],original_auditor=paths['original'],own=paths['own'],tests=paths['tests'])
            accepted=call(); self.assertEqual(accepted['original_auditor'],paths['original'])
            self.assertEqual(accepted['correction_diagnosis'],{'diagnosis':paths['diagnosis']})
            mutations=[]
            for field in ('original_parent_freeze','original_result_COMMIT','original_auditor','original_failed_audit_log','corrected_auditor','corrected_tests'):
                changed=copy.deepcopy(authority); changed[field]['sha256']='0'*64; mutations.append(changed)
            for field in ('version','status','correction'):
                changed=copy.deepcopy(authority); changed[field]='incorrect'; mutations.append(changed)
            changed=copy.deepcopy(authority); changed['diagnosis']={}; mutations.append(changed)
            changed=copy.deepcopy(authority); changed['scope']['BC_admission_authorized']=True; mutations.append(changed)
            changed=copy.deepcopy(authority); changed['unexpected']=True; mutations.append(changed)
            for index,value in enumerate(mutations):
                with self.subTest(mutation=index), self.assertRaises(ValueError): call(value)
            with self.assertRaises(ValueError): r.verify_correction_authority(path,'0'*64,rc=root,
                original_freeze=paths['freeze'],original_commit=paths['commit'],original_auditor=paths['original'],own=paths['own'],tests=paths['tests'])
            call(); (root/'diagnosis').write_text('changed after authority publication')
