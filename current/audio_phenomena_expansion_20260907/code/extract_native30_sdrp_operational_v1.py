...
VERSION = 'extract_native30_sdrp_operational_v1'
...
def load_operational():
    return o.load_module(HERE / 'resume_native30_gpu_guard_v2.py', OPERATIONAL_SHA, 'resume_native30_gpu_guard_v2')


def completion_epoch(completion, bindings, root):
    """Bind the operational epoch without relabeling historical recovery."""
    root = physical.safe_path(root); authority = bindings['operational_parent_freeze']
    backend = load_operational()
    require(completion.get('version') == backend.VERSION and
            completion.get('status') == backend.STATUS and
            completion.get('base_recovery_v2_status') == 'partial_preserved_without_own_completion' and
            completion.get('base_freeze') == bindings['recovery_parent_freeze'] and
            completion.get('base_driver') == bindings['recovery_runner'] and
            completion.get('placement') == backend.PLACEMENT and
            completion.get('operational_parent_freeze') == {'path': authority['path'], 'sha256': authority['sha256']},
            'exact operational completion epoch authority required')
    require(completion.get('driver_execution') == base.file_binding(root / 'driver_run.json') and
            completion.get('blocked_attempt') == base.file_binding(root / 'blocked_attempt.json'),
            'separate operational driver/blocked-attempt bindings required')
    return {**{k: completion[k] for k in ('version', 'status', 'base_recovery_v2_status',
            'operational_parent_freeze', 'base_freeze', 'base_driver', 'placement',
            'driver_execution', 'blocked_attempt', 'replacement_attempt_096')},
            'environment_transition': load_recovery().ENVIRONMENT_TRANSITION}


def item_epoch(contract, mapped, audited):
    provenance = mapped['beat_provenance']; path = mapped['beat_receipt_path']
    epoch = provenance.get('execution_epoch')
    require(epoch in ('preserved_recovery_v2', 'operational_dispatch_v2'), 'explicit operational execution epoch required')
    if epoch == 'preserved_recovery_v2':
        require('physical_dispatch' not in provenance, 'historical item cannot acquire operational dispatch')
        execution = audited['stage_executions'].get(path)
        require(execution is None or execution.get('execution_epoch') == epoch and execution.get('dispatch') is None,
                'historical execution epoch changed')
        return epoch, None
    dispatch = provenance.get('physical_dispatch'); execution = audited['stage_executions'].get(path)
    require(dispatch and execution and execution.get('execution_epoch') == epoch and execution.get('dispatch') == dispatch,
            'new item requires audited physical dispatch and execution')
    require(base.file_binding(dispatch['path']) == dispatch and
            Path(dispatch['path']).parent == Path(contract['operational_root']) / 'dispatch',
            'operational dispatch binding/root changed')
    envelope = base.read_json(dispatch['path'])
    require(set(envelope) == {'payload', 'receipt_sha256'} and
            envelope['receipt_sha256'] == base.value_hash(envelope['payload']), 'dispatch envelope')
    record = envelope['payload']; authority = contract['bindings']['operational_parent_freeze']
    require(record['status'] == 'operational_physical_dispatch' and
            record['driver'] == contract['bindings']['operational_runner'] and
            record['parent_freeze'] == {'path': authority['path'], 'sha256': authority['sha256']} and
            record['host'] == load_operational().HOST and record['placement'] == load_operational().PLACEMENT and
            record['stage'] == 'beats' and mapped['cohort_row']['id'] in record['item_ids'],
            'operational dispatch authority/item changed')
    return epoch, dispatch
...
def prepare(freeze_path, freeze_sha):
    ...
    expected = {'runner': (HERE / (VERSION + '.py'), None), 'tests': (HERE / ('test_' + VERSION + '.py'), None),
                ...
                'operational_runner': (HERE / 'resume_native30_gpu_guard_v2.py', OPERATIONAL_SHA),
    ...
    authority = bindings['operational_parent_freeze']
    operational, prior, effective, cohort, _, _ = load_operational().setup_context(authority['path'], authority['sha256'], audio=False)
    continuation = operational['_base']
    require(operational['base_freeze'] == bindings['recovery_parent_freeze'] and
            operational['base_driver'] == bindings['recovery_runner'] and
            operational['driver'] == bindings['operational_runner'], 'operational/base recovery authority join')
    ...
    completion_path = Path(operational['output_root']) / 'completion.json'
    ...
    require(completion.get('status') == load_operational().STATUS and
    ...
    epoch = completion_epoch(completion, bindings, operational['output_root'])
    ...
    require(output.parent.is_dir() and not any(output.is_relative_to(Path(root)) or Path(root).is_relative_to(output)
            for root in (prior['output_root'], continuation['output_root'], operational['output_root'])) and
    ...
    contract = {'version': VERSION, 'status': 'frozen_CPU_recovery_aware_extraction_only', 'duration_s': 30,
                ...
                'operational_root': operational['output_root'],
...
def verify_completed_inference(contract):
    authority = contract['bindings']['operational_parent_freeze']
    proof = load_operational().verify_completion(authority['path'], authority['sha256'])
    require(proof['status'] == 'verified_complete_native30_operational_continuation' and
    ...
    epoch = completion_epoch(completion, contract['bindings'], contract['operational_root'])
    require(epoch == contract['recovery_epoch'], 'frozen operational epoch changed')
    ...
    return {'verification': proof, 'evidence': evidence, 'beat_provenance': completion['beat_provenance'],
            ...
            'stage_executions': completion['stage_executions']}
...
def inspect_inputs(contract, mapped, audited):
    ...
    epoch, dispatch = item_epoch(contract, mapped, audited)
    if provenance['kind'] == 'ordinary_v2':
        ...
        return {**inputs, 'beat_provenance': provenance, 'physical_dispatch': dispatch}
    ...
    authority = contract['bindings']['recovery_parent_freeze' if epoch == 'preserved_recovery_v2' else 'operational_parent_freeze']
    validator = contract['bindings']['recovery_runner' if epoch == 'preserved_recovery_v2' else 'operational_runner']
    ...
    require(epoch == 'preserved_recovery_v2' or validation['raw_authority_epoch'] == 'v2_recorded',
            'new operational raw evidence must be recorded, not historical')
    require(validation == {'validator': validator,
    ...
    return {'input': source, 'neural_products': products, 'beat_stage_receipt': binding, 'extractor_input_hashes': paths,
            'beat_provenance': provenance, 'recovery_evidence': record, 'recovery_validation': validation, 'physical_dispatch': dispatch}


def recheck_item_inputs(inputs):
    o.recheck_item_inputs(inputs)
    dispatch = inputs.get('physical_dispatch')
    if dispatch:
        require(base.file_binding(dispatch['path']) == dispatch, 'operational dispatch changed during extraction')
...
def run_contract(contract, extractor, bias, audit_callback=verify_completed_inference):
    ...
    with ExitStack() as stack:
        for root in sorted({contract['inference_root'], contract['recovery_root'], contract['operational_root']}):
        ...
        manifest = {'version': VERSION, 'status': 'all_items_processed_scientific_missingness_retained', 'contract_sha256': sha,
                    ...
                    'recovery_epoch': after['recovery_epoch'], 'stage_executions': after['stage_executions'],
