#!/usr/bin/env python3
"""Schema v2: explicitly unavailable old families are not model candidates.
...
def _validate_family_spec(code: str, spec: Any, *, new: bool) -> dict[str, Any]:
    ...
    allowed_states = {"available", "planned"}
...
def active_old_families(config: dict[str, Any]) -> tuple[str, ...]:
    return tuple(code for code in OLD_FAMILY_ORDER if config["old_families"][code]["state"] == "available")
...
def build_evaluation_plan(config: dict[str, Any]) -> pd.DataFrame:
    old_combos = nonempty_combinations(active_old_families(config))
...
def run_dev(
    ...
    manifest = {
        ...
        "active_old_families": list(active_old_families(config)),
        "unavailable_old_families": [code for code in OLD_FAMILY_ORDER if config["old_families"][code]["state"] == "planned"],
