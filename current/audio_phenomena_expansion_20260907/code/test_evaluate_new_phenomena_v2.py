#!/usr/bin/env python3
"""Synthetic-only contract tests for evaluate_new_phenomena_v2.py."""
...
import evaluate_new_phenomena_v2 as evaluator
import prepare_evaluation_inputs_v2 as preparer


HERE = Path(__file__).resolve().parent
PROGRAM = HERE / "evaluate_new_phenomena_v2.py"
PREPARER = HERE / "prepare_evaluation_inputs_v2.py"
...
class NewPhenomenaEvaluationTest(unittest.TestCase):
    ...
    def make_preparation_inputs(self, root: Path, duration: int) -> dict[str, Path]:
        extraction = root / f"extraction_{duration}s"
        extraction.mkdir()
        ids = [f"item_{index}" for index in range(4)]
        contract_hash = f"synthetic-contract-{duration}"
        metadata = pd.DataFrame({
            "id": ids,
            "source_group": ["h1", "h2", "a1", "a2"],
            "group_id": [f"g{index}" for index in range(4)],
            "role": ["development"] * 4,
            "duration_view": [f"{duration}s"] * 4,
            "available": [1] * 4,
            "label": [0, 0, 1, 1],
        })
        metadata_path = root / f"metadata_{duration}s.csv"
        ...
        feature_rows = pd.DataFrame({
            ...
            "M_x": [0.3, 0.4, 0.6, 0.7],
            "M_status": (["missing_short_duration"] * 4 if duration == 10 else ["ok"] * 4),
        ...
        (extraction / "contract.json").write_text(json.dumps({
            "duration": float(duration), "preflight_only": False,
            "contract_hash": contract_hash, "selected_ids": ids,
            "feature_names": {"F": ["F_x"], "H": ["H_x"], "M": ["M_x"]},
            "code_sha256": {},
        }), encoding="utf-8")
        (extraction / "summary.json").write_text(json.dumps({
            "complete_accounting": True, "contract_hash": contract_hash,
            "expected": len(ids), "features_csv_sha256": feature_hash,
        }), encoding="utf-8")
        audit_path = root / f"audit_{duration}s.json"
        audit_path.write_text(json.dumps({
            "passed": True, "errors": [], "row_count": len(ids),
            "extraction_contract_hash": contract_hash,
        }), encoding="utf-8")
        result = {"extraction": extraction, "audit": audit_path, "metadata": metadata_path}
        if duration == 10:
            old_config = root / "old_families.json"
            old_config.write_text(json.dumps({
                "feature_sets": {"common8_10s": {
                    "selection_eligible": True,
                    "eligibility": {"equals": {"duration_view": "10s", "available": 1}},
                    "status_columns": ["feature_status"],
                    "families": {"S": ["s"], "D": ["d"], "R": ["r"], "P": ["p"]},
                }}
            }), encoding="utf-8")
            old_features = root / "old_features_10s.csv"
            pd.DataFrame({
                "item_id": ids, "s": [1, 2, 3, 4], "d": [1, 2, 3, 4],
                "r": [1, 2, 3, 4], "p": [np.nan] * 4,
                "feature_status": ["complete"] * 4,
            }).to_csv(old_features, index=False)
            result.update({"old_config": old_config, "old_features": old_features})
        return result
    ...
    def test_planned_p_yields_exact_sdrfh_lattice_without_cohort_shrink(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            metadata, features, config_path = self.make_inputs(root)
            payload = json.loads(config_path.read_text())
            payload["old_families"]["P"] = {"state": "planned", "columns": []}
            payload["new_families"]["M"] = {
                "state": "planned", "columns": [],
                "phenomenon": "long-range motif recurrence",
            }
            ...
            config = evaluator.load_family_config(config_path)
            plan = evaluator.build_evaluation_plan(config)
            self.assertEqual(evaluator.active_old_families(config), ("S", "D", "R"))
            self.assertEqual(evaluator.active_new_families(config), ("F", "H"))
            self.assertEqual(plan.combination.nunique(), 31)
            self.assertEqual(len(plan), 52)  # 7 old + 3 new + 2*(7*3) matched arms.
            self.assertNotIn("P", set("+".join(plan.combination).split("+")))

            output = root / "dev_sdrfh"
            result = subprocess.run(
                self.command(
                    "dev", metadata, features, config_path, output, "--synthetic-test-only"
                ),
                check=False, text=True, capture_output=True,
            )
            self.assertEqual(result.returncode, 0, result.stderr)
            written_plan = pd.read_csv(output / "evaluation_plan.csv", keep_default_na=False)
            registry = pd.read_csv(output / "cohort_registry.csv")
            self.assertEqual(written_plan.combination.nunique(), 31)
            self.assertEqual(len(registry), 1)
            self.assertEqual(int(registry.iloc[0]["rows"]), 24)

    def test_all_old_planned_and_fhm_available_runs_seven_candidates(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            metadata, features, config_path = self.make_inputs(root, motif_eligibility=False)
            payload = json.loads(config_path.read_text())
            for code in evaluator.OLD_FAMILY_ORDER:
                payload["old_families"][code] = {"state": "planned", "columns": []}
            config_path.write_text(json.dumps(payload), encoding="utf-8")
            config = evaluator.load_family_config(config_path)
            plan = evaluator.build_evaluation_plan(config)
            ...
            self.assertEqual(plan.combination.nunique(), 7)
            ...
            self.assertEqual(manifest["active_old_families"], [])
            self.assertEqual(manifest["unavailable_old_families"], ["S", "D", "R", "P"])
            self.assertEqual(manifest["unique_candidates"], 7)

    def test_prepare_10s_is_atomic_refuses_existing_and_builds_31_candidates(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            paths = self.make_preparation_inputs(root, 10)
            output = root / "prepared_10s"
            command = [
                sys.executable, str(PREPARER), "--duration", "10",
                "--extraction-dir", str(paths["extraction"]),
                "--extraction-audit", str(paths["audit"]),
                "--metadata", str(paths["metadata"]),
                "--old-family-config", str(paths["old_config"]),
                "--old-features", str(paths["old_features"]),
                "--output-dir", str(output),
            ]
            result = subprocess.run(command, check=False, text=True, capture_output=True)
            self.assertEqual(result.returncode, 0, result.stderr)
            config = evaluator.load_family_config(output / "families_10s_v2.json")
            plan = evaluator.build_evaluation_plan(config)
            self.assertEqual(plan.combination.nunique(), 31)
            self.assertEqual(evaluator.active_old_families(config), ("S", "D", "R"))
            self.assertEqual(config["old_families"]["P"]["state"], "planned")
            self.assertEqual(config["new_families"]["M"]["state"], "planned")
            audit = json.loads((output / "preparation_audit.json").read_text())
            self.assertFalse(audit["labels_used_for_feature_selection"])
            self.assertFalse(audit["scoring_authorized"])
            repeated = subprocess.run(command, check=False, text=True, capture_output=True)
            self.assertNotEqual(repeated.returncode, 0)
            self.assertIn("Refusing existing output directory", repeated.stderr)

    def test_prepare_60s_is_pure_fhm_with_seven_candidates(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            paths = self.make_preparation_inputs(root, 60)
            output = root / "prepared_60s"
            result = subprocess.run([
                sys.executable, str(PREPARER), "--duration", "60",
                "--extraction-dir", str(paths["extraction"]),
                "--extraction-audit", str(paths["audit"]),
                "--metadata", str(paths["metadata"]),
                "--output-dir", str(output),
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            config = evaluator.load_family_config(output / "families_60s_v2.json")
            plan = evaluator.build_evaluation_plan(config)
            self.assertEqual(evaluator.active_old_families(config), ())
            self.assertEqual(evaluator.active_new_families(config), ("F", "H", "M"))
            self.assertEqual(len(plan), 7)
