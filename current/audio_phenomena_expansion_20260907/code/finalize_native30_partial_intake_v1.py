...
def verify(root, diagnosis_path, diagnosis_sha, run_path, run_sha, pilot, pilot_audit):
    ...
    inputs = {"contract": a.bind_file(root / "contract.json", CONTRACT),
              ...
              "finalizer_tests": a.bind_file(Path(__file__).resolve().with_name("test_finalize_native30_partial_intake_v1.py")),
    ...
    for path, digest in declared.items():
        ...
        upstream[path] = a.bind_file(path, digest)
    ...
    manifest = {"version": "finalize_native30_partial_intake_v1", "status": STATUS, "contract_sha256": CONTRACT,
...
def finalize(root, diagnosis_path, diagnosis_sha, run_path, run_sha, pilot, pilot_audit, output):
    ...
    with lock.open("rb") as stream:
        ...
        inventory(root, successes, failures)
        commit = {"status": STATUS, "version": "finalize_native30_partial_intake_v1", "contract_sha256": CONTRACT,
