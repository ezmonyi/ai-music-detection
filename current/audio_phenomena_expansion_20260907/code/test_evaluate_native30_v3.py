...
import evaluate_native30_v3 as m
import evaluate_native30_v2 as old
...
class EvaluatorTests(unittest.TestCase):
    ...
    def test_v3_pins_and_v2_numerical_receipt_parity_all_three_modes(self):
        self.assertEqual(m.io.digest(Path(old.__file__)),
                         '586d6d99d63fd4c43684a7fd5a781a7fcdf674c3352ca5f9b4a434d9ccd4ae4b')
        self.assertEqual(m.PREPARER_NAME, 'prepare_native30_evaluation_inputs_v3')
        self.assertEqual(m.io.digest(m.HERE / (m.PREPARER_NAME + '.py')), m.PREPARER_SHA)
        self.assertEqual((m.BASE_SHA, m.PLANNER_SHA, m.SCHEDULE_SHA, m.SCHEDULE_COMMIT_SHA,
                          m.FEATURES, m.FAMILIES, m.EXPECTED, m.SCOPE),
                         (old.BASE_SHA, old.PLANNER_SHA, old.SCHEDULE_SHA, old.SCHEDULE_COMMIT_SHA,
                          old.FEATURES, old.FAMILIES, old.EXPECTED, old.SCOPE))
        for mode in (m.planner.PRIMARY_MODE, *m.planner.DIAGNOSTIC_MODES):
            task = self.task(mode)
            self.assertEqual(m.model_receipt(self.contract, self.table, task),
                             old.model_receipt(self.contract, self.table, task))
        with self.assertRaises(ValueError):
            old.authorize(self.contract, self.freeze, m.io.digest(self.freeze))

    def test_v3_wrong_assembler_pin_cannot_load(self):
        with self.assertRaisesRegex(ValueError, 'code pin'):
            m.load_module(m.HERE / (m.PREPARER_NAME + '.py'), '0' * 64, '_wrong_v3_pin')
