"""Anonymous fixed-revision acceptance of 452 license-screened FMA stems."""
import argparse
import hashlib
import json
...
from publish_fma_stems_v1 import OUT, PREFIX, NOTICE, BATCH, build
...
def run():
    ...
    assert terminal['status'] == '452_fma_stems_uploaded_hash_verified'
    assert terminal['objects'] == 452
    rows, _ = build()
    ...
    return dict(revision=revision, verified_audio_objects=452,
        original_ids=len({r['id'] for row in rows for r in row['original_records']}),
