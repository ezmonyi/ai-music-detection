"""Bind historical Suno processed inputs to two separately attributed sources."""
import argparse
import hashlib
import json
...
def main(objects, originals, out):
    ...
    source = load(originals, '5ddf39584dea9a8e22e93b0ecf65ddff31b0ac9c554bae9ad35c30958a8ff23b')
    ...
    for obj in objects:
        ...
        assert all(m['source_group'] == 'Suno' for m in obj['memberships'])
        assert not obj['public_paths']
        for uid in selected:
            assert source[uid]['source_collection'] in ['humair_suno','suno_unknown']
            assert source[uid]['publisher_declared_license'] in ['MIT','CC-BY-4.0']
        ids.update(selected)
    ...
    assert len(rows) == 900 and ids == set(source)
    out.mkdir()
    raw = (json.dumps(rows, indent=2)+'\n').encode()
    (out/'objects.json').write_bytes(raw)
    summary = dict(objects=900, original_ids=500, bytes=sum(r['bytes'] for r in rows),
        objects_sha256=hashlib.sha256(raw).hexdigest(),
        source_manifest_sha256='5ddf39584dea9a8e22e93b0ecf65ddff31b0ac9c554bae9ad35c30958a8ff23b',
