"""Anonymous fixed-revision acceptance of 600 historical MAESTRO views."""
import argparse
import hashlib
import json
...
from publish_maestro_historical_views_v1 import OUT, PREFIX, NOTICE, build
...
def run():
    ...
    assert terminal['status'] == '600_historical_maestro_views_uploaded_hash_verified'
    assert terminal['objects'] == 600 and terminal['original_ids'] == 300
    rows, _ = build()
    receipts = sorted(OUT.glob('batch_*.json'))
    assert [p.name for p in receipts] == [f'batch_{i:03d}.json' for i in range(30)]
    ...
    return dict(revision=revision, verified_audio_objects=600, original_ids=300,
