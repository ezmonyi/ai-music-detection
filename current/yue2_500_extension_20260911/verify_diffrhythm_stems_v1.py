"""Anonymous fixed-revision acceptance of 400 historical maestro stems."""
import argparse
import hashlib
import json
...
from publish_diffrhythm_stems_v1 import OUT, PREFIX, NOTICE, BATCH, build
...
def run():
    ...
    assert terminal['status'] == '400_diffrhythm_stems_uploaded_hash_verified'
    assert terminal['objects'] == 400 and terminal['original_ids'] == 50
    rows, _ = build()
    receipts = sorted(OUT.glob('batch_*.json'))
    assert [p.name for p in receipts] == [f'batch_{i:03d}.json' for i in range(4)]
    ...
    return dict(revision=revision, verified_audio_objects=400, original_ids=50,
