"""Preserve 2400 source-bound historical maestro stem stems."""
import os
...
from publish_maestro_historical_views_v1 import NOTICE as SOURCE_NOTICE

ROOT = Path('/mnt/nfs-code/users/yi/yue2_500_extension_20260911')
OUT = Path('/mnt/nfs-data/users/yi/yue2_500_extension_20260911/hf_maestro_stems_v1')
PREFIX = 'audio/maestro_historical_stems_v1/'
BATCH = 100
NOTICE = '''# Historical maestro separated stem inputs

2400 exact existing Demucs stem files from 300 original recordings.
...
License: CC-BY-NC-SA-4.0. Noncommercial use only. Retain original attribution and
...
def build():
    plan = ROOT / 'maestro_stem_publication_objects_v1.json'
    assert digest(plan) == 'f468e5e625fd92f148929aef5af0055763bda6898e88b6dce5ff97e816d681d8'
    ...
    assert sum(r['bytes'] for r in rows) == 8467305600
    return rows, paths
...
def worker(token):
    ...
    for index, start in enumerate(range(0, len(rows), BATCH)):
        ...
        revision = api.create_commit(repo_id=transport.REPO, repo_type='dataset', operations=ops,
            commit_message=f'Preserve historical maestro stems {start+1}-{start+len(batch)} of 2400').oid
        verify(api, batch, revision)
        save(receipt, dict(revision=revision, files=[r['path'] for r in batch], sha256_verified=True))
        print(f'Uploaded and verified {start+len(batch)}/2400', flush=True)
    save(OUT/'COMMIT.json', dict(status='2400_maestro_stems_uploaded_hash_verified',
        objects=2400, original_ids=300, manifest_sha256=digest(OUT/'manifest.json'), whole_project_complete=False))
