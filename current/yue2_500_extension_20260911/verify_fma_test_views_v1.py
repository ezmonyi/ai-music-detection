"""Anonymous fixed-revision acceptance of 316 license-screened FMA views."""
import argparse
import hashlib
import json
...
from publish_fma_test_views_v1 import OUT, PREFIX, NOTICE, build
...
def run():
    ...
    assert terminal['status'] == '316_fma_test_views_uploaded_hash_verified'
    assert terminal['objects'] == 316
    rows, _ = build()
    receipts = sorted(OUT.glob('batch_*.json'))
    assert [p.name for p in receipts] == [f'batch_{i:03d}.json' for i in range(8)]
    for i, path in enumerate(receipts):
        r = json.loads(path.read_text())
        assert r['sha256_verified'] is True
        assert r['files'] == [x['path'] for x in rows[i*40:(i+1)*40]]
    ...
    return dict(revision=revision, verified_audio_objects=316,
