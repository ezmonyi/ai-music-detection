"""Anonymous fixed-revision acceptance of 2400 historical maestro stems."""
import argparse
import hashlib
import json
...
from publish_maestro_stems_v1 import OUT, PREFIX, NOTICE, BATCH, build
...
def run():
    ...
    assert terminal['status'] == '2400_maestro_stems_uploaded_hash_verified'
    assert terminal['objects'] == 2400 and terminal['original_ids'] == 300
    rows, _ = build()
    receipts = sorted(OUT.glob('batch_*.json'))
    assert [p.name for p in receipts] == [f'batch_{i:03d}.json' for i in range(24)]
    ...
    return dict(revision=revision, verified_audio_objects=2400, original_ids=300,
