"""Anonymous fixed-revision acceptance of 800 additional open-model views."""
import argparse
import hashlib
import json
...
from publish_open_model_test_views_v1 import OUT, PREFIX, NOTICE, build
...
def run():
    ...
    assert terminal['status'] == '800_open_model_test_views_uploaded_hash_verified'
    assert terminal['objects'] == 800 and terminal['original_ids'] == 800
    rows, _ = build()
    receipts = sorted(OUT.glob('batch_*.json'))
    assert [p.name for p in receipts] == [f'batch_{i:03d}.json' for i in range(40)]
    ...
    return dict(revision=revision, verified_audio_objects=800, original_ids=800,
