"""Preserve 312 source-bound historical maestro stem stems."""
import os
...
from publish_ishizaka_test_views_v1 import NOTICE as SOURCE_NOTICE

ROOT = Path('/mnt/nfs-code/users/yi/yue2_500_extension_20260911')
OUT = Path('/mnt/nfs-data/users/yi/yue2_500_extension_20260911/hf_ishizaka_stems_v1')
PREFIX = 'audio/ishizaka_historical_stems_v1/'
BATCH = 100
NOTICE = '''# Historical ishizaka separated stems

312 exact existing Demucs stem files from 39 original recordings.
Transformation: bass/drums/other/vocals separation. No further byte modification
for publication. CC0-1.0 for these specific Kimiko Ishizaka recordings only. No other MusicNet recordings are covered.
...
def build():
    plan = ROOT / 'ishizaka_stem_publication_objects_v1.json'
    assert digest(plan) == 'c2c9c83cfc4e060febc5151106407497fda465d63dbf54e72b37dc4d3ac8c379'
    ...
    assert sum(r['bytes'] for r in rows) == 1100749728
    return rows, paths
...
def worker(token):
    ...
    for index, start in enumerate(range(0, len(rows), BATCH)):
        ...
        revision = api.create_commit(repo_id=transport.REPO, repo_type='dataset', operations=ops,
            commit_message=f'Preserve historical maestro stems {start+1}-{start+len(batch)} of 312').oid
        verify(api, batch, revision)
        save(receipt, dict(revision=revision, files=[r['path'] for r in batch], sha256_verified=True))
        print(f'Uploaded and verified {start+len(batch)}/312', flush=True)
    save(OUT/'COMMIT.json', dict(status='312_ishizaka_stems_uploaded_hash_verified',
        objects=312, original_ids=39, manifest_sha256=digest(OUT/'manifest.json'), whole_project_complete=False))
