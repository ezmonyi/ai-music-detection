"""Anonymous fixed-revision acceptance of 500 attributed MTT test views."""
import argparse
import hashlib
import json
...
from publish_mtt_test_views_v1 import OUT, PREFIX, NOTICE, build
...
def run():
    ...
    assert terminal['status'] == '500_mtt_test_views_uploaded_hash_verified'
    assert terminal['objects'] == 500 and terminal['original_ids'] == 500
    rows, _ = build()
    receipts = sorted(OUT.glob('batch_*.json'))
    assert [p.name for p in receipts] == [f'batch_{i:03d}.json' for i in range(10)]
    ...
    return dict(revision=revision, verified_audio_objects=500, original_ids=500,
