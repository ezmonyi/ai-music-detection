"""Anonymous fixed-revision acceptance of 1,958 historical open-model vocals."""
import argparse
import hashlib
import json
...
from publish_open_model_vocals_v1 import OUT, PREFIX, NOTICE, BATCH, build
...
def run():
    ...
    assert terminal['status'] == '1958_open_model_vocals_uploaded_hash_verified'
    assert terminal['objects'] == 1958 and terminal['original_ids'] == 1000
    rows, _ = build()
    ...
    return dict(revision=revision, verified_audio_objects=1958, original_ids=1000,
