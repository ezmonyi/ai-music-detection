"""Anonymous fixed-revision acceptance of 2000 historical mtt stems."""
import argparse
import hashlib
import json
...
from publish_mtt_stems_v1 import OUT, PREFIX, NOTICE, BATCH, build
...
def run():
    ...
    assert terminal['status'] == '2000_mtt_stems_uploaded_hash_verified'
    assert terminal['objects'] == 2000 and terminal['original_ids'] == 500
    rows, _ = build()
    ...
    return dict(revision=revision, verified_audio_objects=2000, original_ids=500,
