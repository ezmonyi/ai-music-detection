"""Anonymous fixed-revision acceptance of 5500 historical AIME views."""
import argparse
import hashlib
import json
...
from publish_aime_test_views_v1 import OUT, PREFIX, NOTICE, build
...
def run():
    ...
    assert terminal['status'] == '5500_aime_test_views_uploaded_hash_verified'
    assert terminal['objects'] == 5500 and terminal['original_ids'] == 5000
    rows, _ = build()
    receipts = sorted(OUT.glob('batch_*.json'))
    assert [p.name for p in receipts] == [f'batch_{i:03d}.json' for i in range(110)]
    for i, path in enumerate(receipts):
        r = json.loads(path.read_text())
        assert r['sha256_verified'] is True
        assert r['files'] == [x['path'] for x in rows[i*50:(i+1)*50]]
    ...
    return dict(revision=revision, verified_audio_objects=5500, original_ids=5000,
